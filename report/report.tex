...
\begin{itemize}
    \item \textbf{NetworkX:} We convert our custom graph object into a \texttt{networkx.Graph} in \texttt{visualize.py} to use its\\ \texttt{spring\_layout()} function, which calculates 2D node coordinates based on structural edges.
...
\begin{figure}[h!]
    ...
    \caption{A visualization of the English Wikipedia network. The extreme density and interconnectedness of the nodes form massive communities that would have better suited our structural similarity algorithms. (citation needed https://youtu.be/JheGL6uSF-4 ).}
    \label{fig:wikigraph}
\end{figure}
